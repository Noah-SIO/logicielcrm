...
if(isset($_GET['id'])) {
    ...
    if ($utilisateur != NULL) {
        ...
        if ($_SESSION['droit'] == 5){
            echo "<a href='?action=modifProfil&id=".$idFicheProfilAChercher."&nom=".$utilisateur[0]['nom']."&prenom=".$utilisateur[0]['prenom']."&login=".$utilisateur[0]['identifiant'] ."'><button>Modifier la fiche</button></a>";
        }
    } else {
